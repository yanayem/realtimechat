...
const Sidebar = () => {
  ...
  return (
    <div className='flex flex-col w-20 h-screen bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 py-6 items-center'>
      ...
      <div className="mb-10">
        <div className="w-12 h-12 bg-linear-to-tr from-green-400 to-green-600 rounded-2xl flex items-center justify-center shadow-lg shadow-green-500/20 rotate-3 hover:rotate-0 transition-transform cursor-pointer">
        ...
        {sidetools.map((tool) => {
          ...
          return (
            ...
            >
              ...
              {/* Unique Tooltip Label - Shows on Hover */}
              <span className="absolute left-20 scale-0 group-hover:scale-100 transition-all duration-150 origin-left bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-bold px-3 py-2 rounded-xl shadow-2xl z-50 pointer-events-none after:content-[''] after:absolute after:top-1/2 after:right-full after:-translate-y-1/2 after:border-8 after:border-transparent after:border-right-slate-900 dark:after:border-right-white">
                {tool.label}
              </span>
      ...
      <div className="w-full px-3 space-y-4 border-t border-slate-100 dark:border-slate-800 pt-6">
        <Link to="/profile" className="relative flex items-center justify-center group p-3 rounded-2xl text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
          <User size={22} />
          <span className="absolute left-20 scale-0 group-hover:scale-100 transition-all duration-150 origin-left bg-slate-900 text-white text-xs font-bold px-3 py-2 rounded-xl z-50">Profile</span>
        </Link>

        <button onClick={handleLogout} className="relative flex items-center justify-center group p-3 rounded-2xl text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all">
          <LogOut size={22} />
          <span className="absolute left-20 scale-0 group-hover:scale-100 transition-all duration-150 origin-left bg-red-600 text-white text-xs font-bold px-3 py-2 rounded-xl z-50">Exit</span>
        </button>
      </div>
    </div>
  );
