    ...
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-slate-50 to-green-50 dark:from-slate-950 dark:to-slate-900 transition-colors duration-500 p-4">
      <div className="w-full max-w-md p-8 rounded-3xl bg-white/70 dark:bg-slate-900/80 shadow-2xl backdrop-blur-xl border border-white dark:border-slate-800 transition-all">
        ...
        <header className="mb-6 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Join YT Chat to start messaging</p>
        </header>

        <form onSubmit={handleSubmit} className="space-y-4">
          {signupfields.map((field) => {
            // Logic to determine password visibility for each field
            const isPasswordField = field.type === 'password';
            const isConfirmField = field.id === 'confirmPassword';
              ...
              <div key={field.id} className="space-y-1">
                ...
                <div className="relative">
                  <input 
                    ...
                    required
                    className="w-full p-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800 text-slate-900 dark:text-white outline-hidden focus:ring-2 focus:ring-green-400 focus:border-transparent transition-all placeholder:text-slate-400 pr-12"
                  />
                  
                  {isPasswordField && (
                    <button
                      type="button"
                      onClick={() => isConfirmField ? setShowConfirmPassword(!showConfirmPassword) : setShowPassword(!showPassword)}
